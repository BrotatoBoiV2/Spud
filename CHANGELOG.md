# Current Version
**V2.3.7-2026.01.21**
  Name: Potted Field

  - Added `loops` so you can execute code multiple times, with `cut` keyword to end the loop.
  - Added `pot` functions to allow for modular code.
  - Nested logic is supported.
  - Code refactor and cleaning.
  - Introduced `.spud` as another file extension - and the main one.
  - `bloom` keyword is now supported to get output from a `pot`.
  - Arguments are now supported for `pot` functions.

  ***Reached the "Turing Complete" Milestone.***


---

## Previous Versions

**V1.8.6-2026.01.19**
  Name: Life

  - Extra code refactoring and cleaning.
  - Minor bug fix: 
    - Erroneous debug comment.
  - Code blocks initialized by delimiters: '.~' Eyes and `~.` Roots.
  - Included more mathematical operators and conditional checking.
  - Included booleans.

**V1.6.5-2026.01.15**
  Name: Watering

  - Some code clean-up and refactoring.
  - Started the implementation of boolean keywords: `ripe` and `rotten`
  - Some extra error handling.

**V1.5.2-2026.01.15**
  Name: Potato

  - Conditionals have been introduced so there is now
    logic flow and "tildentation" - the act of Sprouting Potato Eyes "~"
    to indicate an indentation for code blocks.
  - Some errors surrounding the new feature is caught.
  - Renamed `but` keyword to `instead` for clarity.
  
**V1.1.2-2026.01.13**
  Name: Pruning

  - I have refactored the code, added more comments and pruned lines
    to <= 80 characters.

**V1.1-2026.01.09**
  Name: The Roots Grow
  
  - New version allows more complex mathematical equations with parentheses.

**V1.0-2026.01.09**
  Name: The Soil
  
  - The `memory.py` file is for handling variable memory in an environment.
  - Some updates to error output.

**V0.8-2026.01.07**
  Name: Growing Roots
  
  - Variables now allow for basic manipulation.
  - String/Integer Concatenation is more explicit with "+" to join the parts.
  - The Part Handler Method was updated to be a bit easier to manage.

**V0.6-2026.01.05**
  Name: Sprouting
  
  - User input is introduced.
  - Variable handling is being worked on.

**V0.5-2026.01.04**
  Name: Seed

  - This version can parse strings and ignore comments.
  - The `say` node is functional and is being ready to be solidified.
